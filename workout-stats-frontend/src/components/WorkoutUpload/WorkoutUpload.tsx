...
import { Exercise, ExerciseSet, WeightUnit, Workout } from "../../models/workout"
...
export function WorkoutUpload(props: WorkoutUploadProps) {
    ...
    const parseGarminActivity = (a: GarminActivity): Workout => {
        ...
        if (a.fullExerciseSets) {
            w.exerciseSets = a.fullExerciseSets.filter(ges => ges.setType === "ACTIVE").map(ges => {
                ...
                es.exercise.displayName = prettifyGarminExerciseName(ges.exercises[0].name);
    ...
    const fileHandler = (workoutFiles: FileList | null, errorHandler: (err: string) => void) => {
        console.log("fileHandler callback")
        const fileReaders: FileReader[] = [];
        if (!(workoutFiles && workoutFiles.length)) {
            return;
        }
        const promises: Promise<GarminActivity[]>[] = Array.from(workoutFiles).map(file => {
        ...
        Promise
            ...
            .catch(reason => {
                console.log(reason);
                errorHandler(`Could not parse the workout files. urrently only files coming from the Garmin Workout Downloader browser extension are supported. Technical error: ${reason}`);
            });
        };
